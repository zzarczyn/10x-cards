...
Interfejs użytkownika aplikacji **10xCards** został zaprojektowany jako nowoczesna aplikacja jednostronicowa (SPA) osadzona w architekturze Astro ("Islands Architecture"). Priorytetem jest szybkość interakcji ("Desktop-first") oraz płynne przejście między trybem tworzenia (Generator) a trybem przeglądania (Baza Wiedzy).
...
*   **Kluczowe informacje:** Formularz logowania (Email/Hasło), przełącznik do Rejestracji, komunikaty błędów walidacji.
...
### 2.2. Dashboard View (Shell)
*   **Ścieżka:** `/` (gdy sesja aktywna)
*   **Główny cel:** Główny kontener aplikacji, zapewniający nawigację i kontekst użytkownika.
*   **Kluczowe informacje:** Header z logo, Avatar użytkownika (Menu), Główny obszar roboczy (Tabs).
...
*   **UX/Dostępność:**
    ...
    *   Tabela z horyzontalnym scrollem (responsywność).
...
4.  **Recenzja (Review Mode):**
    ...
    *   Użytkownik poprawia literówkę w 2. wierszu (edycja in-place).
    *   Użytkownik usuwa 5. wiersz (ikona kosza), bo jest duplikatem.
    *   Użytkownik dodaje ręcznie specyficzną fiszkę przyciskiem "Dodaj wiersz" pod tabelą.
5.  **Zapis:**
    *   Użytkownik klika "Zapisz" przy 1. wierszu -> Ikona zmienia się na "Tick", wiersz staje się read-only.
...
*   **Logo/Brand:** Lewy górny róg (resetuje widok do domyślnego).
*   **User Menu:** Prawy górny róg. Dropdown zawierający:
    *   Informację o zalogowanym użytkowniku (email).
    *   Przycisk "Wyloguj".
...
*   **ReviewTable:** Zaawansowana tabela (React Table) obsługująca:
    ...
    *   Status wiersza (Draft/Saved).
    *   Optymistyczne aktualizacje UI przy zapisie.
*   **ManualAddButton:** Przycisk dodający pusty wiersz `draft` na koniec tabeli.

### Komponenty Bazy Wiedzy
*   **FlashcardGrid:** Responsywny kontener CSS Grid (dostosowujący liczbę kolumn do szerokości ekranu).
